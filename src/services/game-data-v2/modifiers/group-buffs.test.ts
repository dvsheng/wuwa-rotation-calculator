...
vi.mock('../get-capabilities', () => ({
  getConnectedBuffs: (id: number) => {
    const connectedIds: Record<number, number[]> = {
      1412607003: [1412607003, 1412607009, 1412607004, 1412607012],
      1412607004: [1412607003, 1412607009, 1412607004, 1412607012],
    };
    return (connectedIds[id] ?? [id]).map((connectedId) => ({ id: connectedId }));
  },
}));

const makeBuff = (id: number, applicationTagRequirements: string[] = []): Buff =>
  ({
    buffId: id,
...
describe('groupBuffsByConnection', () => {
  it('groups 1412607003 and 1412607004 together — both are listed in ManageBuff parent 1412607009', () => {
    const buff3 = makeBuff(1412607003);
    const buff4 = makeBuff(1412607004);
