...
export const BySkillView = ({ skills }: { skills: Array<Skill> }) => {
  ...
  const tocItems = skillEntries.map(({ skill, entries }) => ({
    id: `skill-${skill.id}`,
    label: skill.name,
    caption: startCase(skill.originType),
    badges: toCapabilityCountBadges(entries),
